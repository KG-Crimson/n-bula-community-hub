...
export function TournamentBracket() {
  return (
    <section id="torneos" className="relative py-24 px-4 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        <div className="text-center mb-14">
          <div className="text-[10px] font-bold tracking-[0.4em] text-gold mb-3">COMPETICIÓN</div>
          <h2 className="font-display text-4xl md:text-6xl text-purple-gradient mb-4">GESTIÓN DE TORNEOS</h2>
          <p className="text-muted-foreground text-sm md:text-base">
            Largo torneo <span className="text-gold">·</span> 16 jugadores o 32 jugadores
          </p>
        </div>

        <div className="card-nebula rounded-3xl p-4 md:p-8 overflow-x-auto">
          <div className="min-w-[900px] grid grid-cols-[1fr_1fr_1fr_1.2fr_1fr_1fr_1fr] gap-3 items-stretch">
            {/* Left side */}
        ...
        <div className="mt-10 flex flex-wrap justify-center gap-3">
          <a href="#" className="btn-gold hover:btn-gold-hover rounded-full px-7 py-3 text-xs font-bold tracking-widest">
            VER PRÓXIMO TORNEO
          </a>
...
function MatchRow({ name, winner, big }: { name: string; winner?: boolean; big?: boolean }) {
  return (
    <div className="flex items-center justify-between gap-2">
      ...
      {winner && <span className="h-1.5 w-1.5 shrink-0 rounded-full bg-gold shadow-[0_0_10px_oklch(0.83_0.16_85)]" />}
    </div>
  );
}
